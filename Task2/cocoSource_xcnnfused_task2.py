...
class GRUCell(nn.Module):
    ...
    def forward(self, x, state_old):
        ...
        reset = torch.mm(x, self.weight_r) + torch.mm(state_old, self.weigth_r) + self.bias_r
        reset = torch.sigmoid(reset)
        update = torch.mm(x, self.weight_u) + torch.mm(state_old, self.weight_u) + self.bias_u
        update = torch.sigmoid(update)
        cand_hidden = torch.mm(x, self.weight_r) + torch.mm(torch.dot(reset, state_old), self.weight) + self.bias
        cand_hidden = torch.tanh(cand_hidden)
        hidden_state_update = torch.dot(update, state_old) + torch.mm((1 - update), cand_hidden)
