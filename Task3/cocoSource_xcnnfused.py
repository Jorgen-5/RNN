    ...
    def __init__(self, config):
        ...
        if True == self.simplifiedrnn:
            ...
            self.rnn = RNN(input_size=self.embedding_size + self.nnmapsize, hidden_state_size=self.hidden_state_sizes,
                           num_rnn_layers=self.num_rnn_layers, cell_type=self.cell_type)
...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_state_size, num_rnn_layers, cell_type='GRU'):
        ...
        if cell_type == 'GRU':
            ...
            self.cells = nn.ModuleList([LSTMCell(hidden_state_size=self.hidden_state_size, input_size=input_size) for i in range(self.num_rnn_layers)])
...
class LSTMCell(nn.Module):
    ...
    def forward(self, x, state_old):
        ...
        memory_cell_ = torch.mul(forget_gate, state_old[:,self.hidden_state_size:]) + torch.mul(input_gate, candidate_memory)
        #memory_cell_tanh = torch.tanh(memory_cell)

        #print("Memory shape: ", memory_cell_tanh.shape)

        state_new = torch.cat((torch.mul(output_gate, torch.tanh(memory_cell)), memory_cell_tanh), dim=1)
